...
public class MiaoshaController implements InitializingBean {
    ...
    /**
     * 获取秒杀的path,并且验证验证码的值是否正确 此处的User从参数解析器中拿到
     */
...
//		}
        ...
        //        TODO 这里查询数据库并不合理  使用set<good_id,userID> 集合保存已经下过商品订单的 userID，当再次下同一个商品时去判断该用户是否存在
    ...
    public String toList(Model model, MiaoshaUser user, @RequestParam("goodsId") Long goodsId) {
        ...
        if (stockcount <= 0) {//失败			库存至临界值;1的时候，此时刚好来了加入10个线程，那么库存就会-10
